...
async def test_on_task_end_noop_when_no_results(monkeypatch) -> None:
    ...
    assert feedbacks == []
||||||| parent of 6725757 (feat(langsmith_hook): on_task_end aggregate metrics; update import to evals.data._utils)
=======
"""Contract tests for the LangSmith Experiment anchor hook.
...
async def test_on_task_end_noop_when_no_results(monkeypatch) -> None:
    ...
    assert "e3" not in hook._sessions
    assert feedbacks == []
>>>>>>> 6725757 (feat(langsmith_hook): on_task_end aggregate metrics; update import to evals.data._utils)
