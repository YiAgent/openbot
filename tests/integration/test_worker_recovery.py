...
Test environment:
  ...
  - ``execute_handler`` is patched to a failing stub for consumer A and
...
async def _one_iteration(
    ...
    *,
    ...
    fake_config = FakeConfigLoader()
    with (
        patch("openbot.infrastructure.queue.worker.execute_handler", new=mock_handler),
        patch("openbot.infrastructure.queue.worker.load_for_repo", new=fake_config.load_for_repo),
    ):
...
async def test_max_attempts_exhausted_sends_to_dlq(
    ...
    assert dlq_fields.get("reason") == "max_attempts_v3", (
        f"DLQ entry reason must be 'max_attempts_v3', got {dlq_fields.get('reason')!r}"
    )
