...
def _aggregate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_kind": "inspect-ai .eval log",
        "scorer": scorer_name,
...
def _aggregate_langsmith_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(run, dict):
        ...
        run_inputs = getattr(run, "inputs", None) or {}
    metadata = run_extra.get("metadata", {})
    ...
    return {
        ...
        "dataset_name": metadata.get("dataset_version", run_inputs.get("dataset_name", "?")),
...
def load_langsmith_payload(client: Any, run_id: str) -> dict[str, Any]:
    """Load one root run plus sample feedback records from LangSmith."""
    run = client.read_run(run_id)
    samples = []
    for feedback in client.list_feedback(run_ids=[run_id]):
        if getattr(feedback, "key", "").startswith("sample::"):
            samples.append(getattr(feedback, "value", {}) or {})
...
def _render_markdown(agg: dict[str, Any]) -> str:
    ...
    if agg["top_cost_samples"]:
        ...
        lines.append("")
    lines.append(
        f"_Generated by `evals/scripts/export_run_summary.py` from the {agg['source_kind']}._"
    )
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--from-langsmith",
        default=None,
        help="LangSmith root run id to summarize directly.",
    )
    parser.add_argument(
        "--out",
        type=pathlib.Path,
    ...
    if args.from_langsmith is not None:
        from langsmith import Client

        payload = load_langsmith_payload(Client(), args.from_langsmith)
        agg = _aggregate_langsmith_payload(payload)
    elif args.from_langsmith_json is not None:
