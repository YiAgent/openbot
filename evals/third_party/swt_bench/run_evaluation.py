...
from .docker_build import BuildMode, start_container
...
from .exec_spec import ExecMode, ExecSpec, make_exec_spec
from .grading import report_results
from .test_spec import TestSpec, make_test_spec
from .utils import (
    close_logger,
    get_log_dir,
    get_test_directives,
    link_image_build_dir,
    log_git_diff,
    setup_logging,
)
...
def extract_model_patch(
    exec_spec: ExecSpec, raw_model_patch: str, patch_types: list[str], build_mode: BuildMode = "api"
) -> str:
    ...
    if os.path.exists(log_dir / "extracted_patch.diff"):
        with open(log_dir / "extracted_patch.diff") as f:
        ...
        try:
            ...
            copy_to_container(
                container,
                log_dir / "raw_model_patch.txt",
                Path("/root/raw_model_patch.txt"),
                build_mode=build_mode,
            )

            requirements_file = Path(
                os.path.join(
                    os.path.dirname(__file__), "auxillary_src", "requirements_extraction.txt"
                )
            )
            copy_to_container(
                container,
                requirements_file,
                Path("/root/requirements_extraction.txt"),
                build_mode=build_mode,
            )

            extraction_file = Path(
                os.path.join(os.path.dirname(__file__), "auxillary_src", "extract_patches.py")
            )
            copy_to_container(
                container, extraction_file, Path("/root/extract_patches.py"), build_mode=build_mode
            )

            checked_exec_run(container, "pip3 install -r /root/requirements_extraction.txt")
            res = container.exec_run(
                f"python3 /root/extract_patches.py --patch_type {' '.join(patch_types)} --reference_commit {exec_spec.base_commit}"
            )
            if res.exit_code == 0:
...
def eval_in_container(
    ...
    logger.info(f"Eval script for {instance_id} written to eval.sh, now applying to container...")
    copy_to_container(container, eval_file, Path("/eval.sh"), build_mode=build_mode)
    if compute_coverage:
        copy_to_container(
            container,
            Path(os.path.join(os.path.dirname(__file__), "auxillary_src", "trace.py")),
            Path("/root/trace.py"),
            build_mode=build_mode,
        )
...
def eval_in_container_with_diff(
    log_dir: Path,
    container: Container,
    logger: logging.Logger,
    eval_script: str,
...
def run_instance(
    ...
    if model_patch:
        ...
        output_paths = []
        for cld, test_patch, code_patch, patch_id in zip(
            caching_log_dir, test_patches, code_patches, patch_ids, strict=False
        ):
...
def run_eval_exec_spec(
    ...
    try:
        ...
        test_output_path = eval_in_container_with_diff(
            ...
            container,
            logger,
            exec_spec.eval_script,
            exec_spec.timeout,
            instance_id,
            exec_spec.compute_coverage,
            build_mode=build_mode,
        )
...
def make_run_report(
    ...
    for instance in tqdm(dataset):
        ...
        if report_file.exists():
            ...
            directive_paths = [
                gold_test_directive_path,
                gold_test_directive_path,
                model_test_directive_path,
                model_test_directive_path,
            ]
            output_paths = [
                get_log_dir(run_id, patch_id, instance_id) / "test_output.txt"
                for patch_id in patch_ids[:2]
            ] + [
                get_log_dir(patch_id, instance_id, directive_path) / "test_output.txt"
                for patch_id, directive_path in zip(patch_ids[2:], directive_paths, strict=False)
            ]
    ...
    from .constants import EVALUATION_RESULTS_DIR

    report_file = EVALUATION_RESULTS_DIR / (
