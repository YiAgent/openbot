    ...
    * Returns new patch
    ...
    for patch in PATCH_PATTERN.findall(model_patch):
        ...
        for hunk in PATCH_HUNK_PATTERN.findall(patch):
            ...
            subpatches.append(
                FuzzyPatch(
                    change_start, preceding_lines, deleted_lines, inserted_lines, following_lines
                )
            )
...
def apply_fuzzy_patches(
    fuzzy_patch: list[FuzzyFilePatch], testbed: str, patch_type: str = "fuzzy"
) -> bool:
    """
...
class ReplaceFunctionTransformer(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        if (
            ...
            and isinstance(self.new_ast, ast.FunctionDef)
            and node.name == self.new_ast.name
        ):
    ...
    def visit_ClassDef(self, node):
        if (
            ...
            and isinstance(self.new_ast, ast.ClassDef)
            and node.name == self.new_ast.name
        ):
            self.any_change_applied = True
            return self.new_ast
...
def apply_custom_patches(
    custom_patches: list[CustomPatch], testbed: str, patch_type: str = "custom"
) -> bool:
    """
    ...
    for file_name, patches in patches_by_file.items():
        ...
        for patch in patches:
            ...
            if patch.patch_type == "rewrite":
                ...
                if not (
                    isinstance(patch_ast, ast.FunctionDef) or isinstance(patch_ast, ast.ClassDef)
                ):
...
def overlap_score(a: list[str], b: list[str]):
    ...
    for j, context_line in enumerate(a):
        ...
        score += (
            (1 - (distance / max(len(b[j]), len(context_line))))
            if len(b[j]) > 0 or len(context_line) > 0
            else 0
        )
    return score


def write_diff_and_reset(
    testbed: str, reference_commit: str = "", target_file: str = "./processed_patch.diff"
):
...
def main():
    ...
    parser.add_argument(
        "--model_output_file",
        default="/root/raw_model_patch.txt",
        type=str,
        help="Path to raw model output",
    )
    parser.add_argument("--testbed", default="/testbed/", type=str, help="Path to raw model output")
    parser.add_argument(
        "--patch_type",
        nargs="+",
        choices=["vanilla", "fuzzy", "custom"],
        type=str,
        help="Type of patch to be extracted",
    )
    parser.add_argument(
        "--reference_commit", required=True, type=str, help="Type of patch to be extracted"
    )
    parser.add_argument(
        "--target_file",
        default="/root/extracted_patch.diff",
        type=str,
        help="Path to raw model output",
    )
