...
def main(
    ...
    fields = (
        ...
        else ["Applicability (W)", "Success Rate (S)", "F->X", "F->P", "P->P"]
    )
    rows = [["Method", name]]
    ...
    reports = collect_reports(
        model,
        run_id,
        instance_log_path.parent.parent,
        mode=mode,
        seeds=seeds,
        libro_decision_file=libro_decision_file,
    )
    ...
    rows.extend(
        [key, val] for key, val in zip(fields, [applied, success, ftx, ftp, ptp], strict=False)
    )
    ...
    if gold_reports:
        fields = (
            ...
            if format.startswith("latex")
                ...
                "Coverage Delta (Δᵃˡˡ)",
                "Coverage Delta Resolved (Δᔆ)",
                "Coverage Delta Unresolved (Δⁿᵒᵗ ᔆ)",
            ]
        )
        ...
        resolved_coverage_delta = (
            100 * sum_coverage_delta(resolved_reports) / countable_resolved
            if countable_resolved > 0
            else 0
        )
        unresolved_coverage_delta = (
            100
            * sum_coverage_delta(unresolved_reports)
            / (total_coverage_possible - countable_resolved)
            if total_coverage_possible - countable_resolved > 0
            else 0
        )
        rows.extend(
            [key, val]
            for key, val in zip(
                fields,
                [total_coverage_delta, resolved_coverage_delta, unresolved_coverage_delta],
                strict=False,
            )
        )
