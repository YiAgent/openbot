...
from .log_parsers import MAP_REPO_TO_PARSER, parse_log_reproduction_script
from .utils import get_log_dir, setup_logging
...
def get_logs_eval(
    ...
    log_parser = (
        ...
        if exec_mode != "reproduction_script"
        else parse_log_reproduction_script
    )
...
def get_resolution_success(
    report_pred: dict[str, list[str]], report_base: dict[str, list[str]]
) -> tuple[bool, int]:
    """
...
def extract_changed_lines_from_patch(
    patch: str,
...
def extract_executable_lines(
    lines: list[tuple[str, int]], coverage_results: list[dict[str, dict[int, int]]]
) -> list[tuple[str, int]]:
    executable_lines = []
    joined_coverage = {}
...
def get_coverage_delta(
    ...
    coverage_pre: [dict[str, list[tuple[int, int]]]],
    coverage_post: [dict[str, list[tuple[int, int]]]],
...
def get_restricted_coverage(
    lines: list[tuple[str, int]], coverage: [dict[str, list[tuple[int, int]]]]
) -> dict[str, dict[int, int]]:
    restriced_coverage = {}
    for line in lines:
        file_name = line[0]
...
def count_covered_lines(
    coverage_removed: dict[str, dict[int, int]], coverage_added: dict[str, dict[int, int]]
) -> int:
    return sum(
        sum(coverage_removed[k][kk] > 0 for kk in coverage_removed[k].keys())
        for k in coverage_removed
    ) + sum(
        sum(coverage_added[k][kk] > 0 for kk in coverage_added[k].keys()) for k in coverage_added
    )
