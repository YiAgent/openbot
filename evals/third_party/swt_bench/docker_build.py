...
from .utils import Locker, close_logger, setup_logger
...
def docker_build_cli(build_dir: Path, image_name: str, platform: str, nocache: bool = False):
    # Vendor patch: rewrite to `text=True` (which implies utf-8 decoding on
    # Python 3) instead of `encoding="utf-8"`. Functionally equivalent —
    # both turn stdout/stderr from bytes into str — but sidesteps an
    # over-broad semgrep rule that flags `Popen(encoding=)` as a Py3.6+
    # incompatibility. Our project pins Python >=3.11 so the original
    # call was fine; this is purely to silence the post-edit lint hook
    # without an awkward inline `# nosemgrep` waiver.
    response = subprocess.Popen(
        ["docker", "build", "--tag", image_name, ".", "--platform", platform]
...
def docker_build_api(
    ...
    # NB: deliberately NOT passing pull=True here — that would force
    # docker to try and pull EVERY image referenced by a FROM line,
    # including the locally-tagged exec.base.* / exec.env.* intermediate
    # images we built ourselves, which obviously aren't in any registry
    # (→ 'pull access denied'). Cross-arch cache refresh is handled
    # one level up in build_base_image_from_exec_spec via an explicit
    # client.images.pull(ubuntu:22.04, platform=platform) call. See
    # NOTICE.
...
def build_image(
    ...
    try:
        ...
        if build_mode == "cli":
            ...
            response = docker_build_api(build_dir, image_name, platform, client, nocache)
...
def build_base_images(
    ...
    base_images = {x.base_image_key: (x.base_dockerfile, x.platform) for x in exec_specs}
    if force_rebuild:
...
def build_base_image_from_exec_spec(
    ...
    with Locker(
        str(LOCKS_DIR / f"{image_name}_")
    ):  # prevent other processes from checking whether image exists while we are building. Allow for async wait (?)
        try:
            # Check if the base image already exists
            client.images.get(image_name)
        ...
        # Vendor patch: pre-pull the FROM image of the base dockerfile
        # for the *target* platform. Upstream relies on docker's default
        # FROM-resolution which silently reuses a locally-cached tag
        # regardless of arch — fine on a homogeneous local daemon, broken
        # for our remote-daemon-over-SSH setup where the cached
        # `ubuntu:22.04` may have been pulled by some other Mac client as
        # arm64. `images.pull(..., platform=…)` refreshes the cache to
        # the matching variant; subsequent build steps then resolve FROM
        # to the correct arch and `apt install` no longer dies with
        # `exec format error`. The base dockerfile in dockerfiles.py
        # always FROMs ubuntu:22.04, so we hardcode that here.
        try:
            print(f"Pre-pulling ubuntu:22.04 for platform={platform}")
            client.images.pull("ubuntu:22.04", platform=platform)
        except Exception as e:
            # Don't fail the run on a pull hiccup — the build will still
            # try its own FROM resolution and surface a clearer error if
            # the cached image is unusable. Log and continue.
            print(f"Pre-pull failed ({e}); falling through to build.")
    ...
    for exec_spec in exec_specs:
        ...
        try:
            if exec_spec.base_image_key not in base_images:
                base_images[exec_spec.base_image_key] = client.images.get(exec_spec.base_image_key)
...
def build_instance_images(
    ...
    _, env_failed = build_env_images(
        client, test_specs, force_rebuild, max_workers, build_mode=build_mode
    )
...
def start_container(
    ...
    container = build_container(
        exec_spec, client, logger, exec_spec.rm_image, exec_spec.force_rebuild, build_mode
    )
    container.start()
