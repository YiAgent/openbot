...
def copy_to_container(container: Container, src: Path, dst: Path, build_mode: BuildMode):
    ...
    if build_mode == "cli":
        ...
        write_to_container_cli(
            container, compressed_obj_base64, pathlib.Path(f"{dst}.b64"), build_mode
        )
...
def remove_image(client, image_id, logger=None):
    ...
    try:
        ...
        # Vendor patch: idempotent no-op when the image is already gone.
        # Upstream raises here in `quiet` mode, which breaks the
        # `force_rebuild=True` first-run path — that path unconditionally
        # calls remove_image() before ever building, so the very first
        # invocation always throws 404. The semantic of "force rebuild"
        # is "make sure no stale image exists"; ImageNotFound means we
        # already satisfy that precondition.
        log_info(f"Image {image_id} already absent (force-rebuild no-op).")
