...
class FixDataset(EvalDataset):
    ...
    def classify(report: dict[str, Any]) -> Iterable[tuple[str, float, str]]:
        # SWE-bench harness schema_version 2 uses *_ids suffixes.
        # Support both old (resolved/unresolved/error) and new (resolved_ids/…) keys
        # so writeback works against reports generated by any harness version.
    ...
    def build_task(
        ...
        *,
        ...
        # Pre-generate the experiment name once so the predictions filename
        # (run_label) and the LangSmith project name (metadata key
        # "langsmith_experiment_name", consumed by langsmith_hook._build_session)
        # are identical — no more timestamp skew between the two systems.
        ...
        return Task(
            ...
            metadata={
                ...
                # Consumed by langsmith_hook._build_session to use the same name
                # as the predictions file above — single source of truth.
